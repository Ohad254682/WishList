...
export function getBook(index) {
    if (index === -1) {
        return Promise.resolve(gBooks[gBooks.length - 1]);
    }
    if (index === gBooks.length) {
        return Promise.resolve(gBooks[0]);
    }
    return Promise.resolve(gBooks[index]);
}

export function getListLength() {
    return Promise.resolve(gBooks.length);
}

export function addToWishList(book) {
    gWishList.push(book);
    return Promise.resolve(gWishList);
}

export function removeFromWishList(bookTitle) {
    var indexBook = gWishList.map((book) => book.title).indexOf(bookTitle);
    gWishList.splice(indexBook, 1);
    return Promise.resolve(gWishList);
}
export function setAsUnchecked(bookTitle) {
    var indexBook = gBooks.map((book) => book.title).indexOf(bookTitle);
    gBooks[indexBook].isChecked = false;
    return Promise.resolve(gBooks[indexBook]);
}
export function setAsChecked(bookTitle) {
    var indexBook = gBooks.map((book) => book.title).indexOf(bookTitle);
    gBooks[indexBook].isChecked = true;
    return Promise.resolve(gBooks[indexBook]);
}
